...
for ($i = 0; $i < $this->tmpl['countcategories']; $i++) {
	...
	if (isset($this->categories[$i]->extpic) && $this->categories[$i]->extpic) {
		...
		echo JHtml::_( 'image', str_replace('phoca_thumb_m','phoca_thumb_l',$this->categories[$i]->linkthumbnailpath), str_replace('&raquo;','-',$this->categories[$i]->title),array('width' => 300, 'height' => 170,'style' => ''));
	}
