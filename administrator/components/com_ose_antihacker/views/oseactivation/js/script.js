				...
				success: function ( result, request ) {
					...
					if (msg.status!='ERROR')
					{
						Ext.Msg.alert(msg.status, msg.result,function(btn,txt){
							if(btn == 'ok')	{
									window.location='index.php?option=com_ose_antihacker&view=dashboard';
								}
						});
					}
					else
...
Ext.onReady(function(){
	...
	activate.on('click', function(){
		...
		ajaxAction('com_ose_antihacker', 'activate', 'activate', username.dom.value, password.dom.value,'oseath');
	});
});	
