...
//--------------------------- Duplicated Panel------------------------------------ 
    ...
    */
    oseip.dup.dupAcl = new Ext.data.JsonStore({
							...
							fields:['ACLID', 'ACLNAME']
					    });
    
    oseip.dup.combo = new Ext.form.ComboBox({
				    	name: 'Acl Name',
					    typeAhead: true,
					    triggerAction: 'all',
					    lazyRender:true,
					    mode: 'remote',
					    forceSelection: true,
					    valueNotFoundText: ' Please Click Here And Select One IP Acl To Delete',
	...
	oseip.dup.cm = new Ext.grid.ColumnModel({
	        ...
	        columns: [
		        new Ext.grid.RowNumberer(),
		        {
		            ...
		            sortable: true,
		            renderer: function(value){
		            	var combo = oseip.dup.combo;
						var record = combo.findRecord(combo.valueField || combo.displayField, value);
						return record ? record.get(combo.displayField) : combo.valueNotFoundText;
					},
		            editor: oseip.dup.combo,
    ...
    oseip.dup.grid.getSelectionModel().on('selectionchange', function(sm){
       ...
       var s = sm.getSelections();
  		...
  		for(var i = 0, r; r = s[i]; i++){
        	oseip.dup.dupAcl.load({params:{ip:r.data.IP}});
        }
			        ...
			        switch(result){
				        ...
				        default:
				        	//Ext.get('headerpanel').hide();
				           	oseip.app.setAlert('Error',result);
				           	break;
			    	}
			    	oseip.dup.store.reload(); 
			    },
