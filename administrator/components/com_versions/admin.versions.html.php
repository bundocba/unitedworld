...
class HTML_versions{
	...
	function showVersions(&$rows,&$pagenav,&$lists){
			...
			<table>
				<tr>
					...
					<td nowrap="nowrap">
						...
						?>
						Type: 
						<select onchange="document.adminForm.submit();" name="type" id="type" value="type">
							<option value="0" <?php if($type==0){ echo 'selected'; } ?>>Content</option>
							<option value="1" <?php if($type==1){ echo 'selected'; } ?>>K2</option>
						</select>
					</td>
				</tr>
			</table>
		...
		foreach($rows as $row){
			...
			?>
			<a href="index.php?option=com_k2&view=item&cid=<?php echo $row->content_id; ?>"><?php echo $row->title; ?></a>
			<?php
			}
			?>
			</td>
