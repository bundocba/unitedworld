		...
		if($id > 0){
			...
			if($group_ids > 0 || is_array($group_ids)){
				...
				if(inGroup($group_ids)){
					...
					$query = "SELECT * FROM #__version WHERE type=".$type." AND content_id=" . $id . " $sql AND (autosaved != '1' or autosaved is null) ORDER BY id DESC";
					
				}else{
					
					//no access to staging versions
					$query = "SELECT *,'1' as hide_staged FROM #__version WHERE type=".$type." AND content_id=" . $id . " $sql AND  (autosaved != '1' or autosaved is null) ORDER BY id DESC";
			...
			if($type==0){
				...
				$query = "SELECT * FROM #__k2_items WHERE id=" . $id . " LIMIT 1";				
			}
			$db->setQuery( $query );
			$current = $db->loadObject();
...
function savePrefs(){
	...
	*/
	...
	function showVersions(){
		...
		$type		= $mainframe->getUserStateFromRequest( $option.'.type',		'type',		0,	'cmd' );
		...
		if($user->id > 0){	
			...
			$where = " WHERE type=".$type." AND 1 ";
			
			if($type==0){
				$where .=" AND c.state != -2";
			}else{
				$where .=" AND c.published != -2";
			}
