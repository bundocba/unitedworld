...
class plgButtonVersioning extends JPlugin
{
	...
	{
		if(JRequest::getVar('option','')=="com_content"||JRequest::getVar('option','')=="com_k2"){
	        ...
	        $link = $stub . '?option=com_versions&language_id='.$language_id.'&tmpl=component&type='.$type.'&ename='.$name.'&id=' . $id;
			...
			$db =& JFactory::getDBO();
			$sql = "SELECT count(id) FROM #__version WHERE content_id = $id AND type= $type  AND (autosaved != '1' or autosaved is null)";
			$db->setQuery( $sql );
