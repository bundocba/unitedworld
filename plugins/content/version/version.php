	...
	public function onContentAfterSave($context, &$article, $isNew)
	{
		...
		if($isNew === false){
			 ...
			 if($article->id > 0){
				...
				//delete autosaves
				$sql = "DELETE FROM #__version WHERE type=".$type." AND content_id = $id AND autosaved=1";
				$db->setQuery($sql);
				$db->Query($sql);
		 	...
		 	if(isset($_SESSION['content_id'])){
				...
				if($content_id > 0){
					...
					$sql = "DELETE FROM #__version WHERE type=".$type." AND content_id = $content_id AND autosaved=1";
		...
		if($version_limit > 0 && $article->id > 0){
			...
			$id = (int)$article->id;
			$sql = "SELECT COUNT(id) AS cnt FROM #__version WHERE type=".$type." AND content_id = $id  AND (autosaved != '1' or autosaved is null)";
			...
			if($res->cnt > $version_limit){
				...
				$sql = "DELETE FROM #__version WHERE type=".$type." AND content_id = $id  AND (autosaved != '1' or autosaved is null) ORDER BY id ASC LIMIT $difference";
		...
		if($type==1){
			$vars = Array (
			    ...
			    "alias" => $vars['alias'],
			    "title_alias" => "",
			    "introtext" => $vars['introtext'],
			    "fulltext" => $vars['fulltext'],
			    "state" => $vars['published'],
			    "sectionid" => 0,
			    "mask" => 0,
			    "catid" => $vars['catid'],
			    "created" => $vars['created'],
			    "created_by" => $vars['created_by'],
			    "created_by_alias" => $vars['created_by_alias'],
			    "modified" => $vars['modified'],
			    "modified_by" => $vars['modified_by'],
			    "checked_out" => $vars['checked_out'],
			    "checked_out_time" => $vars['checked_out_time'],
			    "publish_up" => $vars['publish_up'],
			    "publish_down" => $vars['publish_down'],
			    "images" => "",
			    "urls" => "",
			    "attribs" => "",
			    "version" => 0,
